...
public class AnnouncementTitleServiceImpl extends ServiceImpl<AnnouncementTitleMapper, AnnouncementTitle> implements AnnouncementTitleService {
                        ...
                        if (response.getParseVoList() != null && !response.getParseVoList().isEmpty()) {
                            ...
                            for (AnnouncementTitleCrawlVo tender : response.getParseVoList()) {
                                ...
                                if (tender.getUrl() != null && !tender.getUrl().startsWith("http")) {
                                    ...
                                    // 将vo转换成实体类
                                    AnnouncementTitle announcementTitle = new AnnouncementTitle();
                                    BeanUtils.copyProperties(tender, announcementTitle);
                                    ...
                                    if (title != null && title.contains("项目编号:")) {
                                        ...
                                        if (end != -1) {
                                            ...
                                            announcementTitle.setProjectNumber(projectNumber);
                                        }
                                    }
                                    ...
                                    if (releaseDateString != null && !releaseDateString.isEmpty()) {
                                        try {
                                            ...
                                            announcementTitle.setReleaseDate(releaseDate);
                                        } catch (Exception e) {
                                            System.err.println("日期转换失败: " + releaseDateString + " 错误: " + e.getMessage());
                                        }
                                    }
                                    //url如果不存在就保存到数据库
                                    if(announcementTitleMapper.selectByMap(Collections.singletonMap("url", announcementTitle.getUrl())) == null){
                                        announcementTitleMapper.insert(announcementTitle);
                                    }
                            }
                        } else {
